...
rpc_method!(CreateUserMethod, "create_user", |params: Option<serde_json::Value>, id| {
...
rpc_method!(GetServerInfoMethod, "get_server_info", |_params: Option<serde_json::Value>, id| {
...
rpc_method!(PingMethod, "ping", |_params: Option<serde_json::Value>, id| {
    rpc_success!("pong", id)
});
...
async fn main() {
    ...
    let registry = rpc_registry_with_methods![
        CreateUserMethod,
        GetServerInfoMethod,
        PingMethod
    ];
    ...
    let requests = vec![
        ...
        rpc_call_request!("create_user", serde_json::json!({
            "name": "Alice",
            "age": 25,
    ...
    for request in requests {
        let response = registry.call(&request.method, request.params, request.id).await;
        println!(
            "Method: {} -> {}",
            request.method,
            if response.is_error() { "ERROR" } else { "SUCCESS" }
        );
