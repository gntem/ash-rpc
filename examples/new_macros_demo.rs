...
rpc_method!(EchoMethod, "echo", |params: Option<serde_json::Value>, id| {
...
rpc_method!(DivideMethod, "divide", |params: Option<serde_json::Value>, id| {
...
async fn main() {
    ...
    println!("Ping response: {}", serde_json::to_string_pretty(&response).unwrap());
    ...
    println!("Add response: {}", serde_json::to_string_pretty(&response).unwrap());
    ...
    println!("Divide response: {}", serde_json::to_string_pretty(&response).unwrap());
    ...
    let request = RequestBuilder::new("divide")
        ...
        .build();
    let response = registry.call("divide", request.params, request.id).await;
    println!("Divide by zero response: {}", serde_json::to_string_pretty(&response).unwrap());
}